...
title: Auggie CLI
slug: auggie
policy: policies/auggie.yml
---

# Stonewall for Auggie CLI

Stonewall runs Auggie CLI inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Auggie CLI, stonewall.sh provides a specific policy at https://stonewall.sh/policies/auggie.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/auggie.yml
```

It covers only what Auggie CLI needs to start and authenticate:

- **Binaries:** `auggie` and `node` (the runtime).
- **Writable paths:** `~/.augment`, where the login session, settings, cache, rules and commands live.
...
The Auggie CLI policy is meant to be combined with policies for what your project actually needs:
