...
slug: pi
policy: policies/pi.yml
---

# Stonewall for Pi Coding Agent

Stonewall runs Pi Coding Agent inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Pi Coding Agent, stonewall.sh provides a specific policy at https://stonewall.sh/policies/pi.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/pi.yml
```

It covers only what Pi Coding Agent needs to start and authenticate:

- **Binaries:** `pi` and `node` (the runtime).
- **Writable paths:** `~/.pi/agent`, where settings, `auth.json`, sessions, extensions and installed packages live.
...
The Pi Coding Agent policy is meant to be combined with policies for what your project actually needs:
