...
title: Kilo CLI
slug: kilo
policy: policies/kilo.yml
---

# Stonewall for Kilo CLI

Stonewall runs Kilo CLI inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Kilo CLI, stonewall.sh provides a specific policy at https://stonewall.sh/policies/kilo.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/kilo.yml
```

It covers only what Kilo CLI needs to start and authenticate:

- **Binaries:** `kilo`, `kilocode` (the same command under its second name) and `node` (runs the launcher script of the npm package).
- **Readable paths:** `~/.kilo`, where global skills, agents and commands live.
- **Writable paths:** `~/.config/kilo` (config, themes, agents), `~/.local/share/kilo` (credentials and sessions), `~/.local/state/kilo` and `~/.cache/kilo` (state and cache).
...
The Kilo CLI policy is meant to be combined with policies for what your project actually needs:
