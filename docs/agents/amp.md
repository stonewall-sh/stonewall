...
title: Amp
slug: amp
policy: policies/amp.yml
---

# Stonewall for Amp

Stonewall runs Amp inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Amp, stonewall.sh provides a specific policy at https://stonewall.sh/policies/amp.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/amp.yml
```

It covers only what Amp needs to start and authenticate:

- **Binaries:** `amp` (a single native executable, no runtime needed).
- **Writable paths:** `~/.config/amp` (settings and skills), `~/.local/share/amp` (`secrets.json` with the API key from `amp login`) and `~/.amp` (the install and update location).
...
The Amp policy is meant to be combined with policies for what your project actually needs:
