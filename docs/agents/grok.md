...
title: Grok CLI
slug: grok
policy: policies/grok.yml
---

# Stonewall for Grok CLI

Stonewall runs Grok CLI inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Grok CLI, stonewall.sh provides a specific policy at https://stonewall.sh/policies/grok.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/grok.yml
```

It covers only what Grok CLI needs to start and authenticate:

- **Binaries:** `grok` and `bun` (the runtime, only needed for the npm install; the installer script ships a standalone binary).
- **Writable paths:** `~/.grok`, where settings, the API key, workspace trust, the local database and the installed binary live.
...
The Grok CLI policy is meant to be combined with policies for what your project actually needs:
