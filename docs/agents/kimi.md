...
title: Kimi Code CLI
slug: kimi
policy: policies/kimi.yml
---

# Stonewall for Kimi Code CLI

Stonewall runs Kimi Code CLI inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Kimi Code CLI, stonewall.sh provides a specific policy at https://stonewall.sh/policies/kimi.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/kimi.yml
```

It covers only what Kimi Code CLI needs to start and authenticate:

- **Binaries:** `kimi` and `node` (the runtime when installed via npm; the script install is a native binary).
- **Writable paths:** `~/.kimi-code`, where config, sessions, credentials, logs and the update cache live.
...
The Kimi Code CLI policy is meant to be combined with policies for what your project actually needs:
