...
title: Qodo Command
slug: qodo
policy: policies/qodo.yml
---

# Stonewall for Qodo Command

Stonewall runs Qodo Command inside a kernel-enforced sandbox. The agent only sees your project directory
...
For Qodo Command, stonewall.sh provides a specific policy at https://stonewall.sh/policies/qodo.yml. You can
use it by simply including it in your `.stonewall.yml` config, or using the CLI:

```shell
stonewall policy include https://stonewall.sh/policies/qodo.yml
```

It covers only what Qodo Command needs to start and authenticate:

- **Binaries:** `qodo`, `node` (the runtime) and `security` (macOS keychain, used for credentials).
- **Writable paths:** `~/.qodo`, where the API key, device id, skill state and the `~/.qodo/bin` install live.
...
The Qodo Command policy is meant to be combined with policies for what your project actually needs:
